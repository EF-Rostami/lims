...
export function MethodsPage() {
  ...
  return (
    <LimsPageLayout
      ...
      headerExtra={
        <ExcelButtons
          exportFilename="test_methods"
          onExport={async () => {
            const rows = await methodsApi.list({ active_only: false });
            return rows.map((m) => ({ code: m.code, name: m.name, description: m.description ?? "", unit: m.unit ?? "" }));
          }}
          onImport={async (rows) => {
