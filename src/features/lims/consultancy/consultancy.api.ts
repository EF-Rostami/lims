...
export interface PopulateAssessmentResult {
  created: number;
  total: number;
}
...
export type GoLiveSeverity = "BLOCKING" | "WARNING";
...
export interface GoLiveDefinitionWithConfig {
  check_key: string;
  standard: string;
  category: string;
  check_type: string;
  ...
  default_severity: GoLiveSeverity;
  order_index: number;
  effective_threshold: number;
  effective_severity: GoLiveSeverity;
  is_enabled: boolean;
  has_override: boolean;
}

export interface ProjectGoLiveConfigRead {
  check_key: string;
  threshold: number | null;
  severity: GoLiveSeverity | null;
  is_enabled: boolean;
}

export interface ProjectGoLiveConfigUpsert {
  threshold?: number | null;
  severity?: GoLiveSeverity | null;
  is_enabled?: boolean;
}
...
export const consultancyApi = {
  ...
  // Go-Live
  listGoLiveDefinitions: async (projectId: number): Promise<GoLiveDefinitionWithConfig[]> => {
    const res = await limsApi.get<GoLiveDefinitionWithConfig[]>(
      `/consultancy/projects/${projectId}/golive/definitions`,
    );
    return res.data;
  },

  upsertGoLiveConfig: async (
    projectId: number,
    checkKey: string,
    data: ProjectGoLiveConfigUpsert,
  ): Promise<ProjectGoLiveConfigRead> => {
    const res = await limsApi.put<ProjectGoLiveConfigRead>(
      `/consultancy/projects/${projectId}/golive/config/${checkKey}`,
      data,
    );
    return res.data;
  },

  resetGoLiveConfig: async (projectId: number, checkKey: string): Promise<void> => {
    await limsApi.delete(`/consultancy/projects/${projectId}/golive/config/${checkKey}`);
  },

  getReadiness: async (projectId: number): Promise<GoLiveReadinessReport> => {
    const res = await limsApi.get<GoLiveReadinessReport>(
      `/consultancy/projects/${projectId}/readiness`,
    );
    return res.data;
  },

  executeGoLive: async (projectId: number): Promise<ConsultancyProjectRead> => {
    const res = await limsApi.post<ConsultancyProjectRead>(
      `/consultancy/projects/${projectId}/go-live`,
    );
    return res.data;
  },
  ...
  populateAssessment: async (assessmentId: number, frameworkId: number): Promise<PopulateAssessmentResult> => {
    const res = await limsApi.post<PopulateAssessmentResult>(
      `/consultancy/assessments/${assessmentId}/populate`,
